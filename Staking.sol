...
contract JazzStaking is Ownable {
    ...
    event LogForfeit(address indexed recipient, uint256 blueAmount, uint256 jazzAmount);
    ...
    /**
        @notice stake Jazz to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        Jazz.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( Blues.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        Blues.safeTransfer( address(warmupContract), _amount );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve BLUE from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = Blues.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit BLUE in warmup and retrieve Jazz
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint blueBalance = Blues.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  blueBalance);
        Jazz.safeTransfer( msg.sender, info.deposit);
        emit LogForfeit(msg.sender, blueBalance, info.deposit);
    }
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        Blues.safeTransferFrom( msg.sender, address(this), _amount );
        Jazz.safeTransfer( msg.sender, _amount );
        emit LogUnstake(msg.sender, _amount);
    }

    /**
        @notice returns the BLUE index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return Blues.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            Blues.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = Blues.circulatingSupply();
    ...
    /**
        @notice returns contract Jazz holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return Jazz.balanceOf( address(this) ).add( totalBonus );
    }
