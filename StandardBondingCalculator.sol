...
contract TimeBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Jazz) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Jazz), "not a Jazz lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Jazz.decimals() ) ).div( getTotalValue( _pair ) );
    }
}
